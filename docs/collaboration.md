...
1. `detector.detect_markers(frame)` → `list[int]`
...
- [ ] [lane-perception-topic.md](./lane-perception-topic.md) 구조를 확인했는가
- [ ] `/control`을 인지에서 발행하지 않는가
- [ ] `pipeline.py` / `types.py` / 노드를 함부로 건드리지 않았는가
- [ ] PR 템플릿 체크리스트를 채웠는가
...
- [lane-perception-topic.md](./lane-perception-topic.md) — **인지·제어 토픽 구조 (PR 전 필독)** ★
